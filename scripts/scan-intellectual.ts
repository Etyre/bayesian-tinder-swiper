...
const results: { name: string; age: number | null; exception: boolean; pInt: number; pVeg: number; hits: number; id: string; top: string }[] = [];
for (const { d, hits } of shortlist) {
  try {
    ...
    results.push({ name: d.name ?? "?", age: d.age, exception: c.intellectual_exception, pInt: c.intellectual_probability, pVeg: c.probability, hits, id: d.id, top });
...
console.log("\nNear misses by P(exception):");
results.sort((a, b) => b.pInt - a.pInt);
for (const r of results.filter((r) => !r.exception).slice(0, 10)) console.log(`  ${(r.pInt * 100).toFixed(0).padStart(3)}%  ${r.name}, ${r.age ?? "?"}  ${r.top}`);
