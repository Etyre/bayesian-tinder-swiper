...
5. Afterwards, the **Review** tab shows every profile it judged. Filter by "probability ≥ X", by the model's call, or by your swipe, and mark each call Agree or Disagree. The agreement rate tells you whether to move the threshold or prior.

**Review manually** is the alternative flow: the model scores the card on screen and waits for you to press Pass or Like in the dashboard (or the arrow keys), which performs the real swipe. Your swipe is recorded as agree/disagree with the model automatically.
